SELECT events.id, events.uid, events.origin_instance_uid, events.project_id, export_project.uid, events.project_name, CASE WHEN subject_issue.id IS NOT NULL AND subject_issue.project_id <> ? THEN NULL ELSE events.issue_id END, events.issue_uid,
	                 CASE WHEN (peer.id IS NULL AND events.related_issue_id IS NOT NULL) OR (peer.id IS NOT NULL AND peer.project_id <> ?) THEN NULL ELSE events.related_issue_id END,
	                 CASE WHEN (peer.id IS NULL AND events.related_issue_id IS NOT NULL) OR (peer.id IS NOT NULL AND peer.project_id <> ?) THEN NULL ELSE events.related_issue_uid END,
