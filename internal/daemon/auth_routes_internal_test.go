...
func TestSelfAuthenticatedRouteMatcherMatchesOnlyTransportRoutes(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"federation vector lookup", http.MethodPost,
			"/api/v1/projects/7/federation/vectors:lookup", true},
		{"vector lookup rejects get", http.MethodGet,
			"/api/v1/projects/7/federation/vectors:lookup", false},
