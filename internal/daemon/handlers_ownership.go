...
func registerOwnershipHandlers(humaAPI huma.API, cfg ServerConfig) {
	...
	huma.Register(humaAPI, huma.Operation{
		...
		if errors.Is(err, db.ErrAlreadyAssigned) {
			...
			return nil, api.NewError(409, "already_assigned",
...
// reachable through assignmentReplayEventTypes — issue.created, issue.snapshot,
// issue.updated — carries project content that claim-only federation
// enrollments have no read authority over.
...
var assignmentReplayEventTypes = []string{
	"issue.created", "issue.snapshot", "issue.updated", "issue.assigned", "issue.unassigned",
...
func assignmentReplayEvents(
	...
	for i, event := range events {
		...
		if unconditionalAssignmentAnchor(event) {
			anchorIndex = i
		}
	}
	if finalIndex < 0 {
...
func unconditionalAssignmentAnchor(event db.Event) bool {
	switch event.Type {
	case "issue.created", "issue.snapshot", "issue.assigned", "issue.unassigned":
		return true
	case "issue.updated":
		_, ok := db.PayloadMap(jsontext.Value(event.Payload))["owner"]
		return ok
	default:
		return false
	}
