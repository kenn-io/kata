...
func TestFoldClosePreservesOwnerWithoutExpiry(t *testing.T) {
	events := []FoldEvent{
		testEvent("issue.created", 1, `{"uid":"issue-1","title":"task","author":"alice","status":"open"}`),
		testEvent("issue.assigned", 2, `{"owner":"alice","assignment_expires_on":"2026-05-23T12:30:00.000Z"}`),
		testEvent("issue.closed", 3, `{"reason":"done","closed_at":"2026-05-23T12:10:00.000Z"}`),
	}
	closed := FoldEvents(events).Issues["issue-1"]
	assert.Equal(t, new("alice"), closed.Owner)
	assert.Nil(t, closed.AssignmentExpiresOn)

	events = append(events,
		testEvent("issue.reopened", 4, `{"reopened_at":"2026-05-23T12:20:00.000Z"}`),
		testEvent("issue.assignment_expired", 5, `{"previous_owner":"alice","assignment_expires_on":"2026-05-23T12:30:00.000Z"}`),
	)
	reopened := FoldEvents(events).Issues["issue-1"]
	assert.Equal(t, "open", reopened.Status)
	assert.Equal(t, new("alice"), reopened.Owner)
	assert.Nil(t, reopened.AssignmentExpiresOn)
}
