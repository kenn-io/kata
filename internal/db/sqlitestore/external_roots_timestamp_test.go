...
func TestExternalCommentsUseCanonicalSQLiteTimestampOrdering(t *testing.T) {
	...
	require.NoError(t, err)
	issueID := issue.ID
	const importSource = "example-import:comments"
	_, err = store.UpsertImportMapping(ctx, db.ImportMappingParams{
		Source: importSource, ExternalID: "imported-issue", ObjectType: "issue",
	...
	importedAt := middle.Add(50 * time.Microsecond)
	result, _, err := store.ImportBatch(ctx, db.ImportBatchParams{
		ProjectID: project.ID, Source: importSource, Actor: "importer",
		Items: []db.ImportItem{{
			ExternalID: "imported-issue", Title: issue.Title, Body: issue.Body,
			Author: issue.Author, Status: issue.Status,
			CreatedAt: issue.CreatedAt, UpdatedAt: issue.UpdatedAt,
			Comments: []db.ImportComment{{
				ExternalID: "imported-comment", Author: "contributor",
				Body: "Imported later", CreatedAt: importedAt,
			}},
		}},
	})
	require.NoError(t, err)
	assert.Equal(t, 1, result.Comments)
	importedMapping, err := store.ImportMappingBySource(ctx, project.ID, importSource, "comment", "imported-comment")
	require.NoError(t, err)
	require.NotNil(t, importedMapping.CommentID)
	var storedImported string
	require.NoError(t, store.QueryRowContext(ctx,
		`SELECT CAST(created_at AS TEXT) FROM comments WHERE id=?`, *importedMapping.CommentID,
	).Scan(&storedImported))
	assert.Equal(t, importedAt.Format(sqliteCommentTimeFormat), storedImported)
	_, err = store.ExecContext(ctx, `UPDATE comments SET created_at=? WHERE id=?`,
		importedAt.String(), *importedMapping.CommentID)
	require.NoError(t, err)
	...
	assert.Equal(t, []string{"External earlier", "Local middle", "Imported later", "Lifecycle later"}, []string{
		comments[0].Body, comments[1].Body, comments[2].Body, comments[3].Body,
	})
