...
// recomputeEventContentHashesTx repairs retained envelopes whose canonical UID
// references were deliberately scrubbed by a purge in the same transaction.
...
SELECT e.uid, e.origin_instance_uid, p.uid, e.project_name,
 ...
 WHERE e.id = $1`, id).Scan(
		...
		if err != nil {
			return fmt.Errorf("rehash purged event %d: %w", id, err)
		}
