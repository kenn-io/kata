...
func appendAllowedIssueIDsPostgres(conditions *[]string, args *[]any, issueIDs []int64) {
	if issueIDs == nil {
		return
	}
	if len(issueIDs) == 0 {
	...
	*conditions = append(*conditions, fmt.Sprintf("i.id = ANY($%d::bigint[])", len(*args)))
}

func appendAllowedIssueIDsPostgresBuilder(query *strings.Builder, args *[]any, issueIDs []int64) {
	conditions := []string{}
	appendAllowedIssueIDsPostgres(&conditions, args, issueIDs)
...
func (s *Store) IssueScopedMembers(ctx context.Context, scope db.APITokenScope) ([]db.Issue, error) {
	return readIssueScopedMembers(ctx, s, scope)
}

func readIssueScopedMembers(ctx context.Context, query exportQueryer, scope db.APITokenScope) ([]db.Issue, error) {
...
func (s *Store) IssueInScope(ctx context.Context, scope db.APITokenScope, issueIDs ...int64) (bool, error) {
	if len(issueIDs) == 0 {
		return true, nil
	}
	var admitted int
	err := s.QueryRowContext(ctx, issueScopeAncestryCTE+`
 SELECT count(DISTINCT target_id) FROM scope_ancestors WHERE uid = $1`,
		scope.RootIssueUID, scope.ProjectUID, issueIDs).Scan(&admitted)
...
func checkIssueScopeTargets(ctx context.Context, tx db.Transaction, scope db.APITokenScope, issueIDs []int64) error {
	...
	var admitted int
	// Lock the actual ancestor rows and parent edges. A reparent must delete
	// an edge and an archive/move must update an issue, so neither can commit
	// while these targets are being mutated. The locks end with this transaction.
	var lockedIssues, lockedLinks int
	err := tx.QueryRowContext(ctx, issueScopeAncestryCTE+`,
 locked_issues AS (
  SELECT i.id FROM issues i WHERE i.id IN (SELECT id FROM scope_ancestors) ORDER BY i.id FOR SHARE
 ),
 locked_links AS (
  SELECT l.id FROM links l
  WHERE l.type = 'parent' AND l.from_issue_id IN (SELECT id FROM scope_ancestors WHERE uid <> $1)
  ORDER BY l.id FOR SHARE
 )
 SELECT (SELECT count(DISTINCT target_id) FROM scope_ancestors WHERE uid = $1),
 (SELECT count(*) FROM locked_issues), (SELECT count(*) FROM locked_links)`,
		scope.RootIssueUID, scope.ProjectUID, issueIDs).Scan(&admitted, &lockedIssues, &lockedLinks)
	if err != nil {
		return err
	}
...
func appendIssueScopePostgres(query *strings.Builder, args *[]any, scope *db.APITokenScope) {
	if scope == nil {
		return
	}
...
func readIssueScopeIDs(ctx context.Context, query exportQueryer, scope *db.APITokenScope, allowed []int64) ([]int64, error) {
	if scope == nil {
		return allowed, nil
	}
