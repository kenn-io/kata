...
func (s *Store) updateIssueAttribute(
	...
	err := s.withSerializableTx(ctx, func(tx *sql.Tx) error {
		...
		switch column {
		case "owner":
			query = `UPDATE issues SET owner = $1, assignment_expires_on = NULL, updated_at = $2 WHERE id = $3`
		case "priority":
...
func (s *Store) ClaimOwner(ctx context.Context, p db.ClaimOwnerParams) (db.ClaimResult, error) {
	...
	err := s.withSerializableTx(ctx, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx,
			`UPDATE issues SET owner = $1, assignment_expires_on = $2, updated_at = $3 WHERE id = $4`,
