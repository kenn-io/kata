...
func assertIndexColumns(t *testing.T, d *db.DB, indexName string, want ...string) {
	t.Helper()
...
func assertClaimProjectionCounts(ctx context.Context, t *testing.T, d *db.DB, projectID int64, wantClaims, wantPending int) {
	t.Helper()
...
func assertFederationBindingColumn(t *testing.T, d *db.DB, column string) {
	t.Helper()
	var n int
	err := d.QueryRow(
...
func assertFederationSyncStatusColumn(t *testing.T, d *db.DB, column string) {
	t.Helper()
	var n int
	err := d.QueryRow(
...
func assertFederationQuarantineColumn(t *testing.T, d *db.DB, column string) {
	t.Helper()
	var n int
	err := d.QueryRow(
...
func TestFederatedSpokeWriteGatePushDisabledRejectsAndPushEnabledPermits(t *testing.T) {
	cases := map[string]func(context.Context, *db.DB, db.Project, db.Issue, db.Issue) error{
		"create issue": func(ctx context.Context, d *db.DB, p db.Project, _, _ db.Issue) error {
		...
		"edit issue": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
			title := "edited"
		...
		"comment": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
		...
		"label": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
			_, _, err := d.AddLabelAndEvent(ctx, issue.ID, db.LabelEventParams{
				EventType: "issue.labeled",
				Label:     "area:db",
				Actor:     "tester",
		...
		"link": func(ctx context.Context, d *db.DB, p db.Project, issue, peer db.Issue) error {
			_, _, err := d.CreateLinkAndEvent(ctx, db.CreateLinkParams{
				ProjectID:   p.ID,
				FromIssueID: issue.ID,
				ToIssueID:   peer.ID,
		...
		"close": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
			_, _, _, err := d.CloseIssue(ctx, issue.ID, "done", "tester", "", nil)
			return err
		},
		"soft delete": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
			_, _, _, err := d.SoftDeleteIssue(ctx, issue.ID, "tester")
			return err
		},
		"issue metadata": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
		...
		"import batch": func(ctx context.Context, d *db.DB, p db.Project, _, _ db.Issue) error {
		...
		"reopen": func(ctx context.Context, d *db.DB, _ db.Project, issue, _ db.Issue) error {
			_, _, _, err := d.ReopenIssue(ctx, issue.ID, "tester")
			return err
		},
		"project metadata": func(ctx context.Context, d *db.DB, p db.Project, _ db.Issue, _ db.Issue) error {
...
func TestFederatedSpokeRejectsRecurrence(t *testing.T) {
	cases := map[string]func(context.Context, *db.DB, db.Project, db.Recurrence) error{
		"create recurrence": func(ctx context.Context, d *db.DB, p db.Project, _ db.Recurrence) error {
		...
		"patch recurrence": func(ctx context.Context, d *db.DB, _ db.Project, rec db.Recurrence) error {
			title := "patched"
		...
		"delete recurrence": func(ctx context.Context, d *db.DB, _ db.Project, rec db.Recurrence) error {
			return d.SoftDeleteRecurrence(ctx, rec.ID, "tester")
		},
		"materialize recurrence": func(ctx context.Context, d *db.DB, _ db.Project, rec db.Recurrence) error {
			tx, err := d.BeginTx(ctx, nil)
			if err != nil {
				return err
			}
			defer func() { _ = tx.Rollback() }()
			_, err = d.MaterializeNext(ctx, tx, rec.ID, "", "tester")
			return err
		},
	}
...
func upsertTestSpokeFederationBinding(ctx context.Context, t *testing.T, d *db.DB, p db.Project, enabled bool) {
	t.Helper()
	upsertTestSpokeFederationBindingWithPush(ctx, t, d, p, enabled, false)
}

func upsertTestSpokeFederationBindingWithPush(
	ctx context.Context, t *testing.T, d *db.DB, p db.Project, enabled, pushEnabled bool,
) {
	t.Helper()
	_, err := d.UpsertFederationBinding(ctx, db.FederationBinding{
		ProjectID:            p.ID,
...
) (*db.DB, context.Context, db.Project, db.Issue, db.Issue) {
	t.Helper()
	d, ctx, p := setupTestProject(t)
	issue, _, err := d.CreateIssue(ctx, db.CreateIssueParams{
		ProjectID: p.ID,
...
func setupFederationIngestHub(t *testing.T) (*db.DB, context.Context, db.Project, string) {
	t.Helper()
	d, ctx, p := setupTestProject(t)
...
func setupIngestClaimIssue(t *testing.T) (*db.DB, context.Context, db.Project, string, db.Issue, db.Issue) {
	t.Helper()
	d, ctx, p, spokeUID := setupFederationIngestHub(t)
	issue, _, err := d.CreateIssue(ctx, db.CreateIssueParams{
		ProjectID: p.ID,
...
func assertIngestedEventCount(ctx context.Context, t *testing.T, d *db.DB, projectID int64, want int) {
	t.Helper()
	var got int
	require.NoError(t, d.QueryRowContext(ctx, `
		SELECT COUNT(*)
