...
func TestClaimViolationQueriesTreatReleaseEventsAsResolutionBoundary(t *testing.T) {
	tests := []struct {
		name    string
		release func(t *testing.T, d *db.DB, ctx context.Context, p db.Project, issue db.Issue, principal db.ClaimPrincipal)
	}{
		{
			name: "claim.released",
			release: func(t *testing.T, d *db.DB, ctx context.Context, p db.Project, issue db.Issue, principal db.ClaimPrincipal) {
		...
		},
		{
			...
			release: func(t *testing.T, d *db.DB, ctx context.Context, p db.Project, issue db.Issue, _ db.ClaimPrincipal) {
				t.Helper()
...
func assertClaimReleasedAndNotLive(t *testing.T, d *db.DB, claimID int64, issueUID string) {
	t.Helper()
...
func assertClaimReleased(t *testing.T, d *db.DB, claimID int64) {
	t.Helper()
	var released int
	require.NoError(t, d.QueryRow(
...
func assertReleaseReason(t *testing.T, d *db.DB, claimID int64, want string) {
	t.Helper()
	var got string
	require.NoError(t, d.QueryRow(
...
func assertPendingResolved(t *testing.T, d *db.DB, requestUID string) {
	t.Helper()
	var resolved int
	require.NoError(t, d.QueryRow(
...
func assertPendingUnresolved(t *testing.T, d *db.DB, requestUID string) {
	t.Helper()
...
func assertLiveClaimCount(t *testing.T, d *db.DB, issueUID string, want int) {
	t.Helper()
	var got int
	require.NoError(t, d.QueryRow(
		`SELECT COUNT(*) FROM issue_claims WHERE issue_uid = ? AND released_at IS NULL`, issueUID,
...
func assertLiveClaimHolder(t *testing.T, d *db.DB, issueUID, want string) {
	t.Helper()
	var got string
	require.NoError(t, d.QueryRow(
...
func assertLiveClaimRevisionAndExpiry(t *testing.T, d *db.DB, issueUID string, wantRevision int64, wantExpiry time.Time) {
	t.Helper()
	var (
		revision int64
		expires  time.Time
...
func assertLiveClaimUpdatedAt(t *testing.T, d *db.DB, issueUID string, want time.Time) {
	t.Helper()
	var got time.Time
	require.NoError(t, d.QueryRow(
...
func assertLiveReleaseReasonNil(t *testing.T, d *db.DB, issueUID string) {
	t.Helper()
...
func liveClaimID(t *testing.T, d *db.DB, issueUID string) int64 {
	t.Helper()
	var id int64
	require.NoError(t, d.QueryRow(
...
func latestClaimViolationPayload(t *testing.T, d *db.DB) map[string]any {
	t.Helper()
	var raw string
	require.NoError(t, d.QueryRow(`
		SELECT payload
...
func localClaimPrincipal(d *db.DB, holder string) db.ClaimPrincipal {
	return db.ClaimPrincipal{
