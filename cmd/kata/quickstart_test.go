...
func TestQuickstart_ContractPrintsManagedWorkflowWithoutMarkers(t *testing.T) {
	...
	assert.Contains(t, out, "--parent <epic-or-coordinating-issue>")
	assert.Contains(t, out, "Parent links express containment and roll-up only")
	assert.Contains(t, out, "--blocks <dependent> / --blocked-by <prerequisite>")
	assert.Contains(t, out, "Use --related <ref> for context only")
	assert.Contains(t, out, "kata wait observes state; it does not require a dependency edge")
